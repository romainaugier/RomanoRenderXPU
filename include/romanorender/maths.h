...
#define CONSTANTS_NAMESPACE_BEGIN namespace constants {
...
ROMANORENDER_FORCE_INLINE bool isfinitef(const float x) noexcept { return __builtin_isfinite(x) != 0; }
...
ROMANORENDER_FORCE_INLINE float fitf(float s, float a1, float a2, float b1, float b2) noexcept { return b1 + ((s - a1) * (b2 - b1)) / (a2 - a1); }
ROMANORENDER_FORCE_INLINE float fit01f(float x, float a, float b) noexcept { return x * (b - a) + a; }
ROMANORENDER_FORCE_INLINE float lerpf(float a, float b, float t) noexcept { return (1.0f - t) * a + t * b; }
ROMANORENDER_FORCE_INLINE float clampf(float n, float lower = constants::zero, float upper = constants::one) noexcept { return maxf(lower, minf(n, upper)); }
ROMANORENDER_FORCE_INLINE float clampzf(float n, float upper = constants::one) noexcept { return maxf(constants::zero, minf(n, upper)); }
ROMANORENDER_FORCE_INLINE float deg2radf(const float deg) noexcept { return deg * constants::pi / 180.0f; }
ROMANORENDER_FORCE_INLINE float rad2degf(const float rad) noexcept { return rad * 180.0f / constants::pi; }
...
ROMANORENDER_FORCE_INLINE float fmodf(const float x, const float y) noexcept { return ::fmodf(x, y); }
...
ROMANORENDER_FORCE_INLINE float atan2f(const float y, const float x) noexcept { return ::atan2f(y, x); }
...
#if defined(__AVX2__)
ROMANORENDER_FORCE_INLINE float maddf(const float a, const float b, const float c) noexcept { return _mm_cvtss_f32(_mm_fmadd_ss(_mm_set_ss(a),_mm_set_ss(b),_mm_set_ss(c))); }
ROMANORENDER_FORCE_INLINE float msubf(const float a, const float b, const float c) noexcept { return _mm_cvtss_f32(_mm_fmsub_ss(_mm_set_ss(a),_mm_set_ss(b),_mm_set_ss(c))); }
ROMANORENDER_FORCE_INLINE float nmaddf(const float a, const float b, const float c) noexcept { return _mm_cvtss_f32(_mm_fnmadd_ss(_mm_set_ss(a),_mm_set_ss(b),_mm_set_ss(c))); }
ROMANORENDER_FORCE_INLINE float nmsubf(const float a, const float b, const float c) noexcept { return _mm_cvtss_f32(_mm_fnmsub_ss(_mm_set_ss(a),_mm_set_ss(b),_mm_set_ss(c))); }
#else
ROMANORENDER_FORCE_INLINE float maddf(const float a, const float b, const float c) noexcept { return ::fmaf(a, b, c); }
ROMANORENDER_FORCE_INLINE float msubf(const float a, const float b, const float c) noexcept { return a * b - c; } 
ROMANORENDER_FORCE_INLINE float nmaddf(const float a, const float b, const float c) noexcept { return -a * b + c;}
ROMANORENDER_FORCE_INLINE float nmsubf(const float a, const float b, const float c) noexcept { return -a * b - c; }
#endif
