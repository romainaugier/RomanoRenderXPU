...
__device__ __forceinline__ float4 make_float4(const float3& vec) { return make_float4(vec.x, vec.y, vec.z, 0.0f); }
...
__device__ __forceinline__ float length_float4(const float4& a) noexcept { return __sqrtf(dot_float4(a, a)); }

__device__ __forceinline__ float length2_float4(const float4& a) noexcept { return dot_float4(a, a); }
